...
const InitialWrapper = ({ children }) => {
    ...
    useEffect(() => {
        const qucikNoteContact = realm.objectForPrimaryKey(Contact, new BSON.ObjectId("000000000000000000000000"));
        if (!qucikNoteContact) {
            realm.write(() => {
    ...
    const syncContacts = async () => {
        ...
        const loopFunc = async () => {
            ...
            const existingContact = realm.objects("Contact").filtered("id == $0", contact.id)[0];
            if (existingContact) {
                realm.write(async () => {
                    existingContact.name = `${contact?.firstName || ""} ${contact.middleName || ""} ${
                        contact.lastName || ""
                    }`;
                    existingContact.emails = contact?.emails?.map((o) => o?.email) || [];
                    existingContact.phoneNumbers = contact.phoneNumbers?.map((o) => o?.digits) || [];
                ...
                if (contact?.company) {
                    await OrgContactLink(realm, "", String(existingContact?._id), contact.company);
                }
            }
            setTimeout(() => {
