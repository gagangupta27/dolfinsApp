...
const NoteItem = ({
    ...
    const TagSection = () => {
        if (
            note?.mentions &&
            note?.mentions?.some(
                (item) => String(item?.contact?._id || item?.organisation?._id) != String(contact?._id)
            )
        ) {
            return (
                <View style={styles.tagsection}>
                    ...
                    {note?.mentions
                        .filter((item) => String(item?.contact?._id || item?.organisation?._id) != String(contact?._id))
                        .map((item) => (
                            <Text
                                style={styles.individualTag}
                                key={"mentions_" + String(item?.contact?._id || item?.organisation?._id)}
                            >
    ...
    const ImageSection = () => {
        if (note.type == "image" && note?.imageData && note?.imageData?.length > 0) {
            return (
                <ScrollView showsHorizontalScrollIndicator={false} horizontal>
                    {note?.imageData?.map((o, idx) => (
                        <TouchableOpacity
                            ...
                            key={idx}
                        >
                            <Image source={{ uri: o?.uri || "" }} style={{ width: 120, height: 120 }} />
                        </TouchableOpacity>
                    ))}
                </ScrollView>
            );
    ...
    const AudioSection = () => {
        if (note.type == "audio" && note.audioUri) {
            return <AudioPlayer audioUri={note.audioUri} volumeLevels={note.volumeLevels} />;
        } else {
            return <View></View>;
        }
    };
                ...
                <View ref={noteContainerRef} style={styles.notecontainer}>
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                        ...
                        <View style={{ alignItems: "center" }}>
                            ...
                            {showPin && note?._id != -7 && (
                                <TouchableOpacity style={{ paddingVertical: 10 }} onPress={onPinPress}>
                                    <Svg
                                        width="18"
                                        height="18"
                                        viewBox="0 0 24 24"
            ...
            >
                ...
                >
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            track(EVENTS.BUTTON_TAPPED.NAME, {
                                [EVENTS.BUTTON_TAPPED.KEYS.BUTTON_NAME]: BUTTON_NAME.DELETE_NOTE,
                            });
                            onDelete(note._id);
                            handleCloseModal();
                        }}
