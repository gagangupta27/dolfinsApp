        ...
        const buttonClicked = async () => {
            if (content || imageData?.length > 0 || audioUri || document) {
                ...
                if (audioUri) {
                    ...
                    uploadAudioData = await fetch(
                        "https://dolfins-server-development.up.railway.app/api/1.0/user/upload/file",
                        {
                            method: "POST",
                            body: formData,
                            headers: {
                ...
                if (imageData && imageData?.length > 0) {
                    for await (const image of imageData) {
                        ...
                        let tempImageUploadData = await fetch(
                            "https://dolfins-server-development.up.railway.app/api/1.0/user/upload/file",
                            {
                                method: "POST",
                                body: formData,
                        ...
                        tempImageData.push({
                            ...image,
                            imageText: tempImageUploadData?.summary,
                            uri: tempImageUploadData?.s3_url,
                        });
                    }
                ...
                if (document) {
                    ...
                    uploadDocumentData = await fetch(
                        "https://dolfins-server-development.up.railway.app/api/1.0/user/upload/file",
                        {
                            method: "POST",
                            body: formData,
                ...
                addNote(
                    `${content}${
                        uploadAudioData?.summary?.length > 0 ? `\n\nTranscribed Audio: ${uploadAudioData?.summary}` : ""
                    }${
                        tempImageData?.length > 0
                            ? `\n\nImage Description :${tempImageData?.map((o) => `${o?.imageText} \n\n`)}`
                            : ""
                    }${
                        uploadDocumentData?.summary?.length > 0
                            ? `\n\nDocument Text: ${uploadDocumentData?.summary}`
                            : ""
                    }`,
                    mentions,
                    tempImageData,
                    uploadAudioData?.s3_url,
                    uploadAudioData?.summary || uploadDocumentData?.summary,
                    volumeLevels ? volumeLevels : [],
                    { ...document, documentUri: uploadDocumentData?.s3_url }
                );
                ...
                <View style={styles.container}>
                    ...
                    >
                        <View style={{ flex: 1, flexDirection: "column" }}>
                            ...
                            {recording && <Recording volumeLevels={volumeLevels} onStopRecording={onStopRecording} />}
                            {audioUri && <AudioPlayer audioUri={audioUri} volumeLevels={volumeLevels} />}
