...
import { addNoteToCalendar, deleteNote, updateNote } from "../../realm/queries/noteOperations";
...
const CalendarEventScreen = ({ route }) => {
    ...
    const event = useObject(CalendarEvent, new BSON.ObjectId(route?.params?.eventId));
    ...
    const addNoteV2 = async (content, mentions, imageData, audioUri, audioText, volumeLevels, document) => {
        ...
        addNoteToCalendar(realm, new BSON.ObjectId(route?.params?.eventId), newNote);
    };

    const updateNoteV2 = async (noteId, content, mentions, imageData, audioUri, audioText, volumeLevels, document) => {
        const updatedNote = {
            content: content,
            mentions: mentions || [],
            type: imageData?.length > 0 ? "image" : audioUri ? "audio" : document ? "document" : "text",
        ...
        <KeyboardAvoidingView keyboardVerticalOffset={64} behavior={"padding"} style={{ flex: 1 }}>
            <View style={Styles.container}>
                <Header
                    title={event?.title}
                    rightIcons={() => (
                        <TouchableOpacity style={[{ marginLeft: 8 }]} onPress={() => setAddEventModalVisible(true)}>
                            <Svg
                                width="24"
                                height="24"
                                viewBox="0 0 24 24"
                ...
                <View style={{ flex: 1 }}>
                    <NotesList
                        ...
                        ListHeaderComponent={() => (
                            <View style={Styles.box}>
                                ...
                                <View>
                                    ...
                                    {event?.description && (
                                        <View>
                                            <Text style={{ paddingVertical: 5 }}>{event?.description}</Text>
                                        </View>
                                    )}
                                    <Text style={Styles.time}>
                                        {event?.eventStartTime.toDateString()} - {event?.eventEndTime.toDateString()}
                                    </Text>
                        ...
                        notes={storedNotes && storedNotes?.length > 0 && storedNotes?.[0] ? [...storedNotes] : []}
