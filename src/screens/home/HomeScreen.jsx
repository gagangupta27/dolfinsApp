...
import { addContact, deleteContact } from "../../realm/queries/contactOperations";
import { createNoteAndAddToContact, useAllCalendarNotes, useAllContactNotes } from "../../realm/queries/noteOperations";
...
const SearchBar = ({ search, text, setText }) => {
    ...
    return (
        <View style={styles.searchbar}>
            <AntDesign style={{ marginRight: 10 }} name="search1" size={20} color="#A5A6F6" />
            <TextInput style={{ flex: 1 }} placeholder="Search" value={text} onChangeText={setText} />
            {text.length > 0 && (
                <TouchableOpacity
                    onPress={() => {
                        setText("");
...
const CommonComponent = () => {
    ...
    const [contactSettingModalContact, setContactSettingModalContact] = useState(null);
    ...
    const handleSelectContact = async (selectedContacts) => {
        const newContacts = selectedContacts.filter((item) => !contacts.some((c) => c.id == item.id));
    ...
    const addNoteV2 = async (content, mentions, imageData, audioUri, audioText = "", volumeLevels, document) => {
        if (
            !mentions.some(
                (mention) => String(mention?.contact?._id || mention?.organisation?._id) === String(quickNoteRef._id)
            )
        ) {
            mentions.push({
        ...
        const newNote = {
            content: content,
            mentions: mentions || [],
            type: imageData?.length > 0 ? "image" : audioUri ? "audio" : document ? "document" : "text",
            imageData: imageData || [],
    ...
    const search = (text) => {
        if (text) {
            ...
            let workHistoryNotes = contacts
                .map((contact) => {
                    if (contact.linkedinProfileData) {
                        const workHistory = getWorkHistoryList(JSON.parse(contact.linkedinProfileData));
                        if (workHistory && workHistory.toLowerCase().includes(text.toLowerCase())) {
                            return {
                                _id: new BSON.ObjectId(),
            ...
            let summaryNotes = contacts
                .map((contact) => {
                    if (contact.linkedinSummary) {
                        if (contact.linkedinSummary.toLowerCase().includes(text.toLowerCase())) {
                            return {
                                _id: new BSON.ObjectId(),
                                contactId: contact._id,
            ...
            let noteResults = [...workHistoryNotes, ...summaryNotes, ...writtenNotesResults].map((note) => {
            ...
            const searchCalendarResults = calendarEvents.filter((calendarEvent) => {
                ...
                if (calendarEvent?.title) {
                    exists = exists || calendarEvent?.title.toLowerCase().includes(text.toLowerCase());
                }
                if (calendarEvent?.description) {
                    exists = exists || calendarEvent?.description?.toLowerCase()?.includes(text?.toLowerCase());
                }
                ...
                if (calendarEvent?.organizer) {
                    exists =
                        exists || calendarEvent?.organizer?.contact?.name?.toLowerCase().includes(text.toLowerCase());
                }
                return exists;
            });
            ...
            if (searchContactResults.length > 0) {
                ...
                listData.push(...searchContactResults.map((item) => ({ ...item, type: "contact" })));
            }
            if (noteResults.length > 0) {
                listData.push({ type: "header", title: "Notes Results" });
                listData.push(...noteResults.map((item) => ({ ...item, type: "note" })));
            }
    ...
    const onFavPress = (item) => {
        realm.write(() => {
            ...
            // faster operations.
            realm.create("Contact", { ...item, isFavourite: !item?.isFavourite }, "modified");
        });
    };
    ...
    const onPinPress = (item) => {
        realm.write(() => {
            ...
            // "modified" only updates properties that have changed, resulting in
            // faster operations.
            realm.create("Contact", { ...item, isPinned: !item?.isPinned }, "modified");
        });
    };
        ...
        <View style={{ flex: 1, backgroundColor: "white" }}>
                                ...
                                <TouchableOpacity onPress={() => setActiveTab("contacts")}>
                                    <Text
                                        style={{
                                            fontSize: 16,
                                            color: "black",
                                            fontFamily: activeTab === "contacts" ? "WorkSans-Bold" : "WorkSans-Regular",
                                            textDecorationLine: activeTab === "contacts" ? "underline" : "none",
                                        }}
                                    >
                                        Contacts
                                    </Text>
                                ...
                                <TouchableOpacity onPress={() => setActiveTab("calendar")}>
                                    <Text
                                        style={{
                                            fontSize: 16,
                                            color: "black",
                                            fontFamily: activeTab === "calendar" ? "WorkSans-Bold" : "WorkSans-Regular",
                                            textDecorationLine: activeTab === "calendar" ? "underline" : "none",
                                        }}
                                    >
                                        Calendar
                                    </Text>
                                ...
                                <TouchableOpacity onPress={() => setActiveTab("organisation")}>
                                    <Text
                                        style={{
                                            ...
                                            color: "black",
                                            fontFamily:
                                                activeTab === "organisation" ? "WorkSans-Bold" : "WorkSans-Regular",
                                            textDecorationLine: activeTab === "organisation" ? "underline" : "none",
                                        }}
                                    >
                                        Organization
                                    </Text>
                                    ...
                                    <FlatList
                                        ...
                                        renderItem={({ item }) => {
                                            if (item.type === "header") {
                                                ...
                                                return <ContactItem item={item} onLongPress={handleLongPress} />;
            ...
            {!isSearch && (
                <View style={{ position: "relative" }}>
                    ...
                    <NewNoteContainerV2 ref={noteRef} addNote={addNoteV2} contact={quickNoteRef} type={"contact"} />
                </View>
            )}
            <AddOrgModal
