...
export default React.forwardRef(
    ({ percentageProp = 0, showPercentage = true, hideCancel = false, containserStyle = {} }, ref) => {
        ...
        const exportCSV = async () => {
            ...
            let contactJSON = [];
            realm.write(() => {
                const contacts = realm.objects(Contact).filtered("id != $0", "000000000000000000000000");
                ...
                for (const contact of contacts) {
                    try {
                        ...
                        const allNotes = realm.objects(Note).filtered(`_id IN $0`, allContactNoteIds);
        ...
        const importICloud = async () => {
            ...
            await new Promise((r) => setTimeout(r, 1000));
            try {
                await CloudStore.readFile(PathUtils.join(defaultICloudContainerPath, "Documents/backup.json"))
                    .then((res) => {
                        importData(null, res);
                    })
                    .catch((err) => {
        ...
        return (
            ...
            >
                ...
                >
                    <View style={[{ paddingTop: 8, paddingBottom: 20, padding: 16 }, containserStyle]}>
                        <Text
                            style={{
                                fontSize: 20,
                                fontWeight: "700",
                        ...
                        {loading && <ActivityIndicator style={{ paddingVertical: 20 }} color={"gray"} size="large" />}
                        <Text
                            style={{
                                fontSize: 13,
                                textAlign: "center",
                        ...
                        {!hideCancel && (
                            <Buttons text="Cancel" containerStyle={{ width: "100%", marginTop: 50 }} onPress={hide} />
                        )}
                    </View>
