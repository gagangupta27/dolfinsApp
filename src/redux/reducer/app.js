...
export const fetchCalendarEvents = createAsyncThunk("/api/1.0/user/calendar_event/list", async () => {
    const res = await Api.get("/api/1.0/user/calendar_event/list");
    return res.data;
});

export const fetchCalendars = createAsyncThunk("/api/1.0/user/calendar/list", async () => {
...
const appSlice = createSlice({
    ...
    reducers: {
        setAuthData: (state, action) => {
            ...
            if (state.authData && state.authData.idToken) setToken(state.authData.idToken);
        },
    },
