...
const AUTOLINK_TEL_CHECK_R = /tel:/i;
const IMAGE_LINK = "(?:\\[[^\\]]*\\]|[^\\[\\]]|\\](?=[^\\[]*\\]))*";
const IMAGE_HREF_AND_TITLE =
  "\\s*<?((?:[^\\s\\\\]|\\\\.)*?)>?(?:\\s+['\"]([\\s\\S]*?)['\"])?";
const IMAGE_SIZE = "(?:\\s+=([0-9]+)?x([0-9]+)?)?\\)\\s*";

const inlineRegex = (regex) => (source, state) =>
  state.inline ? regex.exec(source) : null;
...
const DefaultRules = Object.freeze(
  mergeRules(
    Object.assign(
      {},
      ...Object.entries(DefaultRenders).map(([nodeKey, render]) => ({
        [nodeKey]: { render: render },
      }))
    ),
    {
      heading: {
      ...
      tel: {
        match: inlineRegex(/^<(\+?\d[\d\s()-]*)>/),
        parse: function (capture, parse, state) {
          var address = capture[1];
          var target = capture[1];

          // Check for a `mailto:` already existing in the link:
          if (!AUTOLINK_TEL_CHECK_R.test(target)) {
            target = "tel:" + target.replaceAll(" ", "");
          }
