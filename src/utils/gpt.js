...
function chatGptStreamWeb(content, sessionId, onMessageReceived) {
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        // Step 1: Send messages to create a query

        const queryParams = encodeURIComponent(content);

        // Step 2: Listen to the response stream for the created query
        const url = `${Api.defaults.baseURL}/api/1.0/user/research/response?content=${queryParams}&sessionId=${sessionId}`;
        const stream = new EventSourcePolyfill(url, {
          headers: {
            "Content-Type": "application/json",
            Accept: "text/event-stream",
...
export { askGPT, chatGpt, chatGptStream, chatGptStreamWeb };
