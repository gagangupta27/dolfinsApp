...
function useCalendarNotes(realm: Realm, calendarEventId: BSON.ObjectId) {
    const noteMaps = useQuery(CalendarEventNoteMap).filtered("calendarEventId == $0", calendarEventId);
...
function useContactNotes(realm: Realm, contactId: BSON.ObjectId) {
    const noteMaps = useQuery(ContactNoteMap).filtered("contactId == $0", contactId);
    const notes = noteMaps.map((noteMap) => {
        const note = realm.objectForPrimaryKey(Note, noteMap.noteId);
        return {
            ...note,
    ...
    realm.write(() => {
        for (const note of notes) {
            try {
                ...
                const existingNote = realm.objectForPrimaryKey("Note", new BSON.ObjectId(note._id));
                if (!existingNote) {
...
function addNoteToContact(realm: Realm, contactId: BSON.ObjectId, noteId: BSON.ObjectId) {
    realm.write(() => {
...
function removeNoteFromContact(realm: Realm, contactId: BSON.ObjectId, noteId: BSON.ObjectId) {
    realm.write(() => {
...
async function deleteNote(realm: Realm, noteId: BSON.ObjectId) {
    await realm.write(async () => {
        const organisationNoteMap = realm.objects("NoteOrganisationMap").filtered("noteId == $0", noteId);
        realm.delete(organisationNoteMap); // Delete the mapping first

        const calendarEventNoteMap = realm.objects("CalendarEventNoteMap").filtered("noteId == $0", noteId);
        realm.delete(calendarEventNoteMap); // Delete the mapping first

        const contactNoteMap = realm.objects("ContactNoteMap").filtered("noteId == $0", noteId);
    ...
    try {
        realm.write(() => {
            const quickNotesIds = realm
                .objects(ContactNoteMap)
                .filtered("contactId == $0", new BSON.ObjectId("000000000000000000000000"))
                .map((o) => o?.noteId);
            const allQuichNotes = realm.objects(Note).filtered(`_id IN $0`, quickNotesIds);
